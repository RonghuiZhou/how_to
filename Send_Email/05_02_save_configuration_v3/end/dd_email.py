...
class DailyDigestEmail:
    ...
    def __load_config(self, file_path='dd_config.json'):
        with open(file_path) as file:
            ...
            self.recipients_list = config['recipient_list']
            self.cc_list = config['cc_list']
            try:
                self.bcc_list = config['bcc_list']
            except Exception as e:
                print(e)
                self.bcc_list = []
            self.sender_credentials_email = config['sender_email'] # your sender email address
            self.sender_credentials_pwd = config['sender_password'] # your sender password
            self.server_smtp = config['server_smtp']
            self.server_port = config['server_port']

            print(f"\nSender: \t{self.sender_credentials_email}\nRecipients: \t{self.recipients_list}"
                  f"\nCC: \t{self.cc_list}\nBCC:\t{self.bcc_list}")
    """
    ...
    def send_email(self):
        ...
        msg['From'] = self.sender_credentials_email
        ...
        with smtplib.SMTP(self.server_smtp, self.server_port) as server:
            server.ehlo()
            server.starttls()

            # no need to login from jnj
            if 'jnj.com' not in self.server_smtp:
                server.login(self.sender_credentials_email,
                             self.sender_credentials_pwd)
