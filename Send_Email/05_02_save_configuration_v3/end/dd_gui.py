...
class DailyDigestGUI:
    ...
    def __init__(self, root):
        ...
        #title_label = ttk.Label(self.__root, text = 'Daily\n\U0001F4DC Digest \U0001F4DC',
        ...
        # GUI listbox for Carbon Copy
        cc_frame = ttk.Frame(self.__root)
        cc_frame.pack(padx = 5, pady = 5)
        self.__add_cc_var = StringVar()
        self.__cc_list_var = Variable()
        self.__build_gui_cc(cc_frame,
                            self.__add_cc_var,
                            self.__cc_list_var)
        ...
        # GUI fields for SMTP server/port
        server_frame = ttk.Frame(self.__root)
        server_frame.pack(padx = 5, pady = 5)
        self.__server_smtp_var = StringVar()
        self.__server_port_var = StringVar()
        self.__build_gui_server(server_frame,
                                self.__server_smtp_var,
                                self.__server_port_var)
        ...
        try: # load values from configuration file
            ...
            self.__add_cc_var.set('')
            self.__cc_list_var.set(self.__email.cc_list)
            
            self.__hour_var.set('07') # default send time
            ...
            self.__server_smtp_var.set(self.__email.server_smtp)
            self.__server_port_var.set(self.__email.server_port)
            self.__sender_email_var.set(self.__email.sender_credentials_email)
            self.__sender_password_var.set(self.__email.sender_credentials_pwd)
    ...
    """
    Build GUI elements to add/remove Carbon Copy 
    """

    def __build_gui_cc(self, master, add_cc_var, cc_list_var):
        # create GUI widgets
        header = ttk.Label(master, text='Carbon Copy:', style='Header.TLabel')
        spacer_frame = ttk.Frame(master)  # used as GUI spacer

        cc_entry = ttk.Entry(master, width=40, textvariable=add_cc_var)
        cc_scrollbar = ttk.Scrollbar(master, orient=VERTICAL)
        cc_scrollbar.grid(row=4, column=1, sticky=N + S + W + E)
        cc_listbox = Listbox(master, listvariable=cc_list_var,
                                     selectmode='multiple', width=40, height=5)
        cc_listbox.configure(yscrollcommand=cc_scrollbar.set)
        cc_scrollbar.config(command=cc_listbox.yview)

        add_button = ttk.Button(master, text='Add Carbon Copy', command=self.__add_cc)
        remove_button = ttk.Button(master, text='Remove Selected',
                                   command=lambda: self.__remove_selected_cc(cc_listbox.curselection()))

        # place GUI widgets using grid geometry manager
        header.grid(row=0, column=0)
        cc_entry.grid(row=1, column=0)
        add_button.grid(row=2, column=0)
        spacer_frame.grid(row=3, column=0, pady=5)
        cc_listbox.grid(row=4, column=0)
        remove_button.grid(row=5, column=0)
    ...
    """
    Build GUI elements to configure SMTP Server
    """
    def __build_gui_server(self, master, server_smtp_var, server_port_var):
        # create GUI widgets
        header = ttk.Label(master, text = 'SMTP Server:', style = 'Header.TLabel')
        smtp_label = ttk.Label(master, text = "SMTP:")
        smtp_entry = ttk.Entry(master, width = 40,
                                textvariable = server_smtp_var)
        port_label = ttk.Label(master, text = 'Port:')
        port_entry = ttk.Entry(master, width = 40, textvariable = server_port_var)

        # place GUI widgets using grid geometry manager
        header.grid(row = 0, column = 0, columnspan = 2)
        smtp_label.grid(row = 1, column = 0, pady = 2, sticky = E)
        smtp_entry.grid(row = 1, column = 1, pady = 2, sticky = W)
        port_label.grid(row = 2, column = 0, pady = 2, sticky = E)
        port_entry.grid(row = 2, column = 1, pady = 2, sticky = W)
    ...
    """
    Callback function to add Carbon Copy
    """
    def __add_cc(self):
        new_cc = self.__add_cc_var.get()
        if new_cc != '':
            cc_list = self.__cc_list_var.get()
            if cc_list != '':
                self.__cc_list_var.set(cc_list + (new_cc,))
            else:
                self.__cc_list_var.set([new_cc])
            self.__add_cc_var.set('') # clear entry field

    """
    Callback function to remove selected Carbon Copy(s)
    """
    def __remove_selected_cc(self, selection):
        cc_list = list(self.__cc_list_var.get())
        for index in reversed(selection):
            cc_list.pop(index)
        self.__cc_list_var.set(cc_list)
    ...
    def __update_settings(self):
        ...
        self.__email.cc_list = list(self.__cc_list_var.get())
        ...
        self.__email.server_smtp = {'SMTP': self.__server_smtp_var.get(),
                                    'Port': self.__server_port_var.get()}
    ...
    def __save_config(self, file_path='dd_config.json'):
        config = {'add_recipient': self.__add_recipient_var.get(),
                  ...
                  'server_smtp': self.__server_smtp_var.get(),
                  'server_port': self.__server_port_var.get(),
    ...
    def __load_config(self, file_path='dd_config.json'):
        with open(file_path) as file:
            ...
            self.__add_cc_var.set(config['add_cc'])
            self.__cc_list_var.set(config['cc_list'])
            ...
            self.__server_smtp_var.set(config['server_smtp'])
            self.__server_port_var.set(config['server_port'])
